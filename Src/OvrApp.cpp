...
RenderFont fontHeader, fontMenu, fontList, fontSlot, fontSmall;
...
jlong
Java_com_a_gear_boy_go_MainActivity_nativeSetAppInterface(JNIEnv *jni,
                                                          jclass clazz,
                                                          jobject activity,
...
OvrApp::OvrApp()
        : SoundEffectContext(NULL), SoundEffectPlayer(NULL), GuiSys(OvrGuiSys::Create()),
          Locale(NULL), SceneModel(NULL) {
    LOG("new gearboycore");
...
void CreateScreen() {
    // emu screen layer
    CylinderSwapChain = vrapi_CreateTextureSwapChain(
            VRAPI_TEXTURE_TYPE_2D, VRAPI_TEXTURE_FORMAT_8888, CylinderWidth, CylinderHeight, 1,
            false);

    textureIdScreen = vrapi_GetTextureSwapChainHandle(CylinderSwapChain, 0);
    glBindTexture(GL_TEXTURE_2D, textureIdScreen);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, CylinderWidth, CylinderHeight, GL_RGBA,
    ...
    // menu layer
    MenuSwapChain = vrapi_CreateTextureSwapChain(
            VRAPI_TEXTURE_TYPE_2D, VRAPI_TEXTURE_FORMAT_8888, menuWidth, menuHeight, 1, false);

    textureIdMenu = vrapi_GetTextureSwapChainHandle(MenuSwapChain, 0);
    glBindTexture(GL_TEXTURE_2D, textureIdMenu);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, menuWidth, menuHeight, GL_RGBA, GL_UNSIGNED_BYTE,
                    NULL);
    ...
    for (int y = 0; y < CylinderHeight; y++) {
        for (int x = 0; x < CylinderWidth; x++) {
            ...
            /* texData[y * CylinderWidth + x] =
                            (gearboy_frame_buf[(y / 2) * VIDEO_WIDTH + (x / 2)].red << 0) |
                            (gearboy_frame_buf[(y / 2) * VIDEO_WIDTH + (x / 2)].green << 8) |
                            (gearboy_frame_buf[(y / 2) * VIDEO_WIDTH + (x / 2)].blue << 16) |
                            (gearboy_frame_buf[(y / 2) * VIDEO_WIDTH + (x / 2)].alpha << 24); */
            memcpy(&texData[y * CylinderWidth + x],
                   &gearboy_frame_buf[(y / 2) * VIDEO_WIDTH + (x / 2)], 4);
        }
    }
...
void UpdateSlotImage() {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, VIDEO_WIDTH, VIDEO_HEIGHT, GL_RGBA,
                    GL_UNSIGNED_BYTE, currentGame->saveStates[saveSlot].saveImage);
    glBindTexture(GL_TEXTURE_2D, 0);
}
...
void LoadRam() {
    std::ifstream file(CurrentRom->SavePath,
...
void SaveState(int slot) {
    ...
    LOG("copy image");
    memcpy(currentGame->saveStates[slot].saveImage,
           gearboy_frame_buf, sizeof(GB_Color) * VIDEO_WIDTH * VIDEO_HEIGHT);
    LOG("update image");
    UpdateSlotImage();
...
void LoadGame(Rom *rom) {
    ...
    for (int i = 0; i < 10; ++i) {
        if (!LoadStateImage(i)) {
            ...
            memset(currentGame->saveStates[i].saveImage,
                   0, sizeof(GB_Color) * VIDEO_WIDTH * VIDEO_HEIGHT);
        } else {
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (ent->d_type == 8) {
                ...
                if (strFilename.find(".gb") != std::string::npos ||
                    ...
                    LOG("found rom: %s %s %s",
                        newRom.RomName.c_str(), newRom.FullPath.c_str(),
...
void OvrApp::EnteredVrMode(const ovrIntentType intentType, const char *intentFromPackage,
    ...
    if (intentType == INTENT_LAUNCH) {
        ...
        InitFontMaster(menuWidth, menuHeight);
        LoadFont(&fontHeader, const_cast<char *>("/system/fonts/Roboto-Regular.ttf"), 55);
        LoadFont(&fontMenu, const_cast<char *>("/system/fonts/Roboto-Light.ttf"), 22);
        LoadFont(&fontList, const_cast<char *>("/system/fonts/Roboto-Light.ttf"), 18);
        LoadFont(&fontSmall, const_cast<char *>("/system/fonts/Roboto-Light.ttf"), 16);
        LoadFont(&fontSlot, const_cast<char *>("/system/fonts/Roboto-Light.ttf"), 26);
        ...
        SceneModel = LoadModelFile(app->GetFileSys(), sceneUri, Scene.GetDefaultGLPrograms(),
                                   materialParms);
...
bool
OvrApp::OnKeyEvent(const int keyCode, const int repeatCount, const KeyEventType eventType) {
...
void MergeAudioBuffer() {
    ...
    for (int i = 0; i < audio_sample_count / 2; ++i) {
        audio_buf[i * 2] = audio_buf[i *
                                     4];// (s16) (((int)audio_buf[i * 4] + (int)audio_buf[i * 4 + 2]) / 2);
        audio_buf[i * 2 + 1] = audio_buf[i * 4 +
                                         1];//(s16) (((int)audio_buf[i * 4 + 1] + (int)audio_buf[i * 4 + 3]) / 2);
    }
    for (int i = 0; i < audio_sample_skip_count / 2; ++i) {
        audio_buf[audio_sample_count / 2 + i * 2] = audio_buf_skip[i *
                                                                   4];// (s16) (((int)audio_buf_skip[i * 4] + (int)audio_buf_skip[i * 4 + 2]) / 2);
        audio_buf[audio_sample_count / 2 + i * 2 + 1] = audio_buf_skip[i * 4 +
                                                                       1];//(s16) (((int)audio_buf_skip[i * 4 + 1] + (int)audio_buf_skip[i * 4 + 3]) / 2);
    }
    */
...
void EmulatorFrame(const ovrFrameInput &vrFrame) {
    ...
    if (vrFrame.Input.buttonState & BUTTON_LEFT_TRIGGER) {
        ...
        core->RunToVBlank(gearboy_frame_buf, audio_buf_skip, &audio_sample_skip_count);
        MergeAudioBuffer();
        */
        core->RunToVBlank(gearboy_frame_buf, audio_buf, &audio_sample_count);
    }
...
void UpdateGUI(const ovrFrameInput &vrFrame) {
    ...
    if (!romSelection) {
        ...
        if (vrFrame.Input.buttonState & BUTTON_LSTICK_UP ||
            ...
            if ((!(lastButtonState & BUTTON_LSTICK_UP) &&
                 !(lastButtonState & BUTTON_DPAD_UP))
            ...
            if ((!(lastButtonState & BUTTON_LSTICK_DOWN) &&
                 !(lastButtonState & BUTTON_DPAD_DOWN))
                || buttonDownCount > SCROLL_DELAY) {
            ...
            if ((!(lastButtonState & BUTTON_LSTICK_LEFT) &&
                 !(lastButtonState & BUTTON_DPAD_LEFT))
            ...
            if ((!(lastButtonState & BUTTON_LSTICK_RIGHT) &&
                 !(lastButtonState & BUTTON_DPAD_RIGHT))
                || buttonDownCount > SCROLL_DELAY_MOVE) {
...
void DrawGUI() {
    ...
    if (currentMenu == &mainMenu && !romSelection)
        DrawRectangle(menuWidth - 240 - 20, HEADER_HEIGHT + 20, 240, 216,
                      {0.1f, 0.1f, 0.1f, 0.040f});
    ...
    RenderText(fontHeader, strHeader, 5.0f, 0.0f, 1.0f, headerColor);
    RenderText(fontSmall, STR_VERSION, menuWidth - strVersionWidth - 5.0f,
               HEADER_HEIGHT - 19, 1.0f, Vector3f(0.8f, 0.8f, 0.8f));

    if (!romSelection) {
        if (currentMenu == &moveMenu) {
            // update the position of the screen
            //RenderText(fontMenu, strMoveMenu, menuWidth / 2 - strMoveMenuTextWidth / 2,
            //           150, 1.0f, Vector3f(1.0f, 1.0f, 1.0f));
        }

        for (uint i = 0; i < currentMenu->MenuItems.size(); i++) {
            RenderText(fontMenu, currentMenu->MenuItems[i].Text,
                       listPosX + 20,
                       85 + menuItemSize * i, 1.0f,
                       ((uint) currentMenu->CurrentSelection == i) ? menuSelectionColor
                                                                   : menuColor);
        }
        ...
        for (uint i = (uint) menuListState; i < menuListState + maxListItems; i++) {
            if (i < romFiles.size()) {
                RenderText(fontList, romFiles[i].RomName, listPosX + scrollbarWidth + 10,
                           listStartY + listItemSize * (i - menuListState), 1.0f,
                           ((uint) currentRomListSelection == i) ? menuSelectionColor
                                                                 : menuColor);
            } else
                break;
        }
...
ovrFrameResult OvrApp::Frame(const ovrFrameInput &vrFrame) {
    ...
    /*
    LOG("orientation: %f, %f, %f, %f", vrFrame.Tracking.HeadPose.Pose.Orientation.x,
        vrFrame.Tracking.HeadPose.Pose.Orientation.y, vrFrame.Tracking.HeadPose.Pose.Orientation.z,
        vrFrame.Tracking.HeadPose.Pose.Orientation.w);

    LOG("position: %f, %f, %f", vrFrame.Tracking.HeadPose.Pose.Position.x,
        vrFrame.Tracking.HeadPose.Pose.Position.y, vrFrame.Tracking.HeadPose.Pose.Position.z);
    */
...
// Add a simple cylindrical layer
    ...
    res.Layers[res.LayerCount++].Cylinder =
            LayerBuilder::BuildCylinderLayer(CylinderSwapChain, CylinderWidth, CylinderHeight,
                                             &vrFrame.Tracking, followHead);
    ...
    if (menuOpen) {
        ...
        res.Layers[res.LayerCount++].Cylinder =
                LayerBuilder::BuildSettingsCylinderLayer(MenuSwapChain, menuWidth, menuHeight,
                                                         &vrFrame.Tracking, followHead);
...
void ChangeDistance(MenuItem *item, float dir) {
    ...
    if (LayerBuilder::radiusMenuScreen < MinRadius)
        LayerBuilder::radiusMenuScreen = MinRadius;
    if (LayerBuilder::radiusMenuScreen > MaxRadius)
        LayerBuilder::radiusMenuScreen = MaxRadius;
...
void ChangeScale(MenuItem *item, float dir) {
    ...
    if (LayerBuilder::screenSize < 0.5f)
        LayerBuilder::screenSize = 0.5f;
    if (LayerBuilder::screenSize > 2.0f)
        LayerBuilder::screenSize = 2.0f;
...
void OvrApp::SetUpMenu() {
    ...
    settingsMenu.MenuItems.push_back({strPalette, OnClickChangePaletteRight,
                                      OnClickChangePaletteLeft, OnClickChangePaletteRight});
    settingsMenu.MenuItems.push_back({strForceDMG[forceDMG ? 0 : 1],
                                      OnClickEmulatedModel, OnClickEmulatedModel,
                                      OnClickEmulatedModel});

    settingsMenu.MenuItems.push_back({strMove[followHead ? 0 : 1],
    ...
    // move menu stuff
    moveMenu.MenuItems.push_back({"Yaw: " + to_string(LayerBuilder::screenYaw),
                                  nullptr, OnClickMoveScreenYawLeft, OnClickMoveScreenYawRight});
    moveMenu.MenuItems.push_back({"Pitch: " + to_string(LayerBuilder::screenPitch),
                                  nullptr, OnClickMoveScreenPitchLeft,
                                  OnClickMoveScreenPitchRight});
    moveMenu.MenuItems.push_back({"Roll: " + to_string(LayerBuilder::screenRoll),
                                  nullptr, OnClickMoveScreenRollLeft, OnClickMoveScreenRollRight});
    moveMenu.MenuItems.push_back({"Distance: " + to_string(LayerBuilder::radiusMenuScreen),
                                  nullptr, OnClickMoveScreenDistanceLeft,
                                  OnClickMoveScreenDistanceRight});
    moveMenu.MenuItems.push_back({"Scale: " + to_string(LayerBuilder::screenSize),
                                  nullptr, OnClickMoveScreenScaleLeft,
                                  OnClickMoveScreenScaleRight});
